...
class NetworkManager {
    ...
    func removeFromCart(productId: Int) async throws {
        guard let request = createRequest("/cart/remove", method: "DELETE", body: try? JSONEncoder().encode([
            "productId": productId
        ])) else {
            throw NetworkError.invalidURL
        }
    ...
    func updateCartQuantity(productId: Int, quantity: Int) async throws {
        guard let request = createRequest("/cart/update", method: "PUT", body: try? JSONEncoder().encode([
            "productId": productId,
            "quantity": quantity
        ])) else {
            throw NetworkError.invalidURL
